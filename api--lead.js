// api/lead.js
// Fetches full lead data from FUB API
// DEPLOY THIS FILE TO: api/lead.js in your Vercel project
...
export default async function handler(req, res) {
  ...
  try {
    const [personRes, notesRes, callsRes, tasksRes, eventsRes] = await Promise.all([
      fetch(`${BASE}/people/${personId}`, { headers }),
    ...
    const [person, notesData, callsData, tasksData, eventsData] = await Promise.all([
    ...
    return res.status(200).json({
      summary,
      person: person || null,
      notes: (notesData.notes || []).map(n => ({ id: n.id, body: n.body, created: n.created, createdBy: n.createdBy })),
      ...
      tasks: (tasksData.tasks || []).map(t => ({ id: t.id, name: t.name, dueDate: t.dueDate, isCompleted: t.isCompleted, assignedTo: t.assignedTo })),
